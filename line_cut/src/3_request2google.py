...
def calculate_line_interval(hojo_name, page):
    #ファイル調達
    with open("../input/images/{0}/p{1}/{0}-p{1}.jpg".format(hojo_name, page), 'rb') as ff:
        content = ff.read()
...
if __name__ == "__main__":
    ...
    for hojo in hojos:
        if not os.path.exists("../output/{}/".format(hojo)):
            os.makedirs("../output/{}/".format(hojo))
        if os.path.exists("../output/{}/rintervals.json".format(hojo)):
            continue
        generate_rintervals(hojo)
